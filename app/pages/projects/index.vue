...
const { data: _allProjects, pending } = await useAsyncData('projects', () => $fetch('/api/projects'))

function getBarFill(s: string) {
  if (s === 'on-track') return 'pfill-green'
  if (s === 'at-risk') return 'pfill-amber'
  if (s === 'delayed') return 'pfill-red'
  if (s === 'not-started') return 'pfill-gray'
  return 'pfill-blue'
}

const allProjects = computed<Proj[]>(() => {
  if (!_allProjects.value) return []
  return _allProjects.value.map((p: any) => ({
    id: p.id,
    key: p.key,
    name: p.name,
    category: p.category,
    ...
    totalT: p.totalT,
    progress: p.progress,
    barFill: getBarFill(p.status),
    dueLabel: p.dueDate,
    overdue: false,
    assignees: p.assignees || [],
    attach: p.attachCount,
        ...
        <div class="proj-card-header">
          ...
          <div class="relative">
            ...
            <div
              ...
              @click.stop
            >
              <button class="proj-action-item" @click.prevent.stop="viewProject(p)">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                View
              </button>
              <button class="proj-action-item" @click.prevent.stop="editProject(p)">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                Edit
              </button>
              <button class="proj-action-item proj-action-item--danger" @click.prevent.stop="deleteProject(p.id)">
